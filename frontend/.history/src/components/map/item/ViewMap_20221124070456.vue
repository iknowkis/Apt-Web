...
export default {
  name: "ViewMap",
  props: ["dongcode", "search", "sido", "receivedSelected","closeStr"],
  data() {
    return {
      markers: [],
      starbucks: [],
  ...
  methods: {
    ...
    removeChipMarker(str) {
      if(str == "스타벅스") {
        ...
        for (let i = 0; i < this.etcs.length; i++) {
          let idx = -1;
          if(this.etcs[i].key == str) {
            console.log("이거 수정해야해" + str);
            for (let j = 0; j < this.etcs[i].data.length; j++) {
              this.etcs[i].data[j].setMap(null);
            }
            idx = i;
          }
          if(idx != -1) {
            this.etcs.splice(i,1);
          }
        }
      }
    },
