...
<div>
  ...
  <div>
      ...
      @click:close="searchObj.starbucks = false">
        <v-avatar left>
      ...
      @click:close="searchObj.school = false">
        <v-avatar left>
      ...
      v-if="searchObj.cafe"
      @click:close="searchObj.cafe = false">
        <v-avatar left>
      ...
      @click:close="searchObj.store = false">
        <v-avatar left>
      ...
      >
        ...
        v-if="item"
        @click:close="searchObj.etc.splice(i,1)">
          {{item}}
        </v-chip>
      </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    searchPlaces() {
      ...
      if (this.search.replace("스타벅스", "").length == this.search.length - 4) {
        this.searchObj.starbucks = true;

      } else if (this.search.replace("스벅", "").length == this.search.length - 2) {
        this.searchObj.starbucks = true;

      } else if (this.search.replace("카페", "").length == this.search.length - 2) {
        this.searchObj.cafe = true;

      } else if (this.search.replace("편의점", "").length == this.search.length - 3) {
        this.searchObj.store = true;

      } else if (this.search.replace("학교", "").length == this.search.length - 2) {
