          ...
          <v-card-title>
            <div>
              ...
              <b>
                ...
                parseInt(selected.recentPrice.replace(",",""))%10000+"천만원") : parseInt(selected.recentPrice.replace(",",""))+"천만원"}}
              </b>
            </div>
          </v-card-title>
          <v-card-text>
...
export default {
  ...
  watch: {
    selected() {
      this.viewRoadmap();
      console.log(this.selected.recentPrice);
      // let range[0] >= 10 ? parseInt(range[0]/10)+"억"+(range[0]%10 == 0 ? "" : range[0]%10+"천만원") : range[0]+"천만원"
    },
  },
  methods: {
    viewRoadmap() {
