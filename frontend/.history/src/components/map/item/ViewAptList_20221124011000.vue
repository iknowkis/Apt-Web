				...
				>
					<v-scale-transition>
						<div>
							...
							<div>
								<b>
									{{parseInt(data.recentPrice.replace(",","")) >= 10000 ?
									parseInt(parseInt(data.recentPrice.replace(",",""))/10000)+"억"+(parseInt(data.recentPrice.replace(",",""))%10000 == 0 ? "" :
									parseInt(data.recentPrice.replace(",",""))%10000+"천만원") : rparseInt(data.recentPrice.replace(",",""))+"천만원"}}
								</b>
							</div>
						</div>
					</v-scale-transition>
