...
public class MemberServiceImpl implements MemberService {
	...
	public MemberDto userInfo(String userid) throws Exception {
		...
		MemberDto result = sqlSession.getMapper(MemberMapper.class).userInfo(userid);
	...
	public MemberDto find(MemberDto memberDto) throws Exception {
		return sqlSession.getMapper(MemberMapper.class).find(memberDto);
	}

	@Override
	public boolean modify(MemberDto memberDto) throws Exception {
		if (sqlSession.getMapper(MemberMapper.class).modify(memberDto) == 1) {
			return true;
		}
		else {
			return false;
